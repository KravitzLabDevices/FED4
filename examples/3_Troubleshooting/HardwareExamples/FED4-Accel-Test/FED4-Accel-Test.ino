/*
  ...
  The accelerometer measures acceleration forces acting on the device.
  - X axis: Left/Right
...
void setup() {
  ...
  Serial.print("Gravity offset calibrated - X:");
...
void updateDisplay() {
  ...
  if (firstDraw) {
    ...
    fed.drawLine(0, 110, 168, 110, DISPLAY_BLACK);
  ...
  fed.fillRect(25, 35, 140, 75, DISPLAY_WHITE);  // Clear X, Y, Z value areas
  fed.fillRect(45, 115, 120, 20, DISPLAY_WHITE); // Clear magnitude value area
  ...
  fed.setCursor(80, 75);
  fed.print("(");
  fed.print(accelY / 9.81, 1);
  fed.print("g)");
  
  // Display Z axis value
  fed.setCursor(30, 100);
  fed.print(accelZ, 1);
  fed.setCursor(80, 100);
  fed.print("(");
  fed.print(accelZ / 9.81, 1);
  fed.print("g)");
  
  // Calculate and display magnitude
  float magnitude = sqrt(accelX * accelX + accelY * accelY + accelZ * accelZ);
  fed.setCursor(50, 130);
  fed.print(magnitude, 1);
  ...
  // Y acceleration moves ball left/right (INVERTED), Z acceleration moves ball up/down
  // Note: Display coordinates have Y increasing downward
  float dt = updateInterval / 1000.0; // Convert to seconds
  
  // Apply acceleration to velocity
  ballVelX -= tiltY * sensitivity * dt;  // Inverted Y for left/right
  ballVelY -= tiltZ * sensitivity * dt;  // Z for up/down (negative because screen Y is inverted)
